...
import Techpack1 from "../../../assets/Enterprice/TechPack1.png";
import { whyTechPackData } from "../../../Data/Techpackdata/TechPackData";
  ...
  return (
    <section className="flex flex-col self-center my-6 md:my-16 text-black w-[90vw]">
      ...
      <p className="mt-6 md:mt-11 font-semibold text-lg md:text-xl text-gray-550 leading-8 md:leading-9 text-center md:text-left">
        A Tech Pack is a detailed document that includes technical sketches,
        measurements, fabric details, trims, stitching guidelines, printing or
        embroidery placements, and packaging instructions. It serves as a
        universal language between designers, developers, and manufacturers,
        ensuring that every piece is produced exactly as envisioned.
      </p>
      ...
      {/* Description */}
      <div className="mt-6 md:mt-11 text-lg md:text-xl lg:text-2xl leading-8 md:leading-9 text-left mb-6">
        <div className="flex flex-col gap-2">
          {whyTechPackData.highlights.map((item, index) => (
            <p key={index}>
              <strong className="mt-6 md:mt-11 font-semibold text-lg md:text-xl text-gray-550 leading-8 md:leading-9 text-center md:text-left">{item.strongText}</strong>
              <span className="mt-6 md:mt-11 font-medium text-lg md:text-xl text-gray-550 leading-8 md:leading-9 text-center md:text-left"> {item.description}</span>
            </p>
          ))}
        </div>
      </div>
